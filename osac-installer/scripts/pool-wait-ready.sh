...
wait_for() {
    local what="$1" resource="$2" condition="$3" timeout="${4:-${TIMEOUT}}"
    log "waiting for ${what}..."
    if ! oc wait --for="${condition}" "${resource}" -n "${NS}" --timeout="${timeout}s" 2>&1; then
...
wait_for "keycloak-database" statefulset/keycloak-database condition=jsonpath='{.status.readyReplicas}'=1 300
