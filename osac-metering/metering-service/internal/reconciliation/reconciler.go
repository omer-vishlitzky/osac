...
func (r *Reconciler) RunFull(ctx context.Context) error {
	...
	for id, fs := range fulfillmentState {
		...
		if !exists {
			ce := buildCorrectionEvent(id, "compute_instance", fs.tenantID,
				MissedCreation, "", fs.state, fs.billingDimensions, nil, now)
			if err := r.publisher.Publish(ctx, ce); err != nil {
				return fmt.Errorf("publishing missed_creation for %s: %w", id, err)
			}
			...
			if err := r.store.Upsert(ctx, newState); err != nil {
				if errors.Is(err, projection.ErrStaleVersion) {
					...
					return fmt.Errorf("upserting missed creation for %s: %w", id, err)
				}
			}
			continue
		}
		...
		if ps.CurrentState != fs.state {
			...
			if err := r.publisher.Publish(ctx, ce); err != nil {
				return fmt.Errorf("publishing state_drift for %s: %w", id, err)
			}
			...
			if err := r.store.Upsert(ctx, ps); err != nil {
				if errors.Is(err, projection.ErrStaleVersion) {
					...
					return fmt.Errorf("upserting state drift for %s: %w", id, err)
				}
			}
			...
			if err := r.publisher.Publish(ctx, ce); err != nil {
				return fmt.Errorf("publishing billing_dimensions_drift for %s: %w", id, err)
			}
			...
			if err := r.store.Upsert(ctx, ps); err != nil {
				if errors.Is(err, projection.ErrStaleVersion) {
					...
					return fmt.Errorf("upserting billing dimensions drift for %s: %w", id, err)
				}
			}
		}
	}
	...
	for id, ps := range projMap {
		if _, exists := fulfillmentState[id]; !exists {
			ce := buildCorrectionEvent(id, ps.ResourceType, ps.TenantID,
				MissedDeletion, ps.CurrentState, "", ps.BillingDimensions, nil, now)
			if err := r.publisher.Publish(ctx, ce); err != nil {
				return fmt.Errorf("publishing missed_deletion for %s: %w", id, err)
			}
			reconCorrections.WithLabelValues(string(MissedDeletion), ps.ResourceType).Inc()
			corrections++

			if err := r.store.Delete(ctx, id); err != nil {
				return fmt.Errorf("deleting missed deletion for %s: %w", id, err)
			}
		}
	}
	...
	if err != nil {
		return fmt.Errorf("loading billable resources for heartbeat check: %w", err)
	}
	...
	for i := range freshProjection {
		...
		if ps.LastHeartbeatAt == nil || now.Sub(*ps.LastHeartbeatAt) > 2*r.heartbeatInterval {
			hbEvent := buildSyntheticHeartbeat(*ps, now)
	...
	if len(heartbeatIDs) > 0 {
		if err := r.store.UpdateLastHeartbeat(ctx, heartbeatIDs, now); err != nil {
			return fmt.Errorf("updating last heartbeat after synthetic heartbeats: %w", err)
		}
	}
...
func buildSyntheticHeartbeat(ps projection.ResourceState, now time.Time) cloudevents.Event {
