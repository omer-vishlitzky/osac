...
func (r *Reconciler) reconcileFulfillmentResources(ctx context.Context, fulfillmentState map[string]fulfillmentResource, projMap map[string]projection.ResourceState, now time.Time) (int, error) {
	...
	for id, fs := range fulfillmentState {
		...
		if !exists {
			...
			if isBillable {
				newState.BillableSince = &transitionTime
				newState.ComponentBillableSince = events.NextComponentBillableSince(nil, nil, fs.billingDimensions, transitionTime)
			}
		...
		if ps.CurrentState != fs.state {
			...
			if isBillable && !wasBillable {
				...
				ps.ComponentBillableSince = events.NextComponentBillableSince(nil, nil, fs.billingDimensions, transitionTime)
			}
			if !isBillable {
