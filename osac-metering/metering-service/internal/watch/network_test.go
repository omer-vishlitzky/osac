...
var _ = Describe("Watch registration", func() {
	It("registers all supported resource payloads", func() {
		...
		Expect(filter).To(Equal("has(event.compute_instance) || has(event.cluster_order) || has(event.external_ip) || has(event.nat_gateway) || has(event.volume) || has(event.bare_metal_instance)"))
